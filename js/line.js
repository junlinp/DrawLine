...
function getTriangleVertex(originRiverData){
    ...
    var points = changeWidth(lines,originRiverData);
    return points;
}
...
function allLines(originRiver){
    ...
    for(var i = 0; i<classes.length; i++){
        ...
        for(var j =0; j<sourceIndex.length; j ++){
            ...
            var wholeLine = getConnectLine(rank, sourceIndex[j]);
...
//获得一条完整河流的所有点的坐标
function getPoints(allLines){
    var lines = [];
    for (var i = 0; i<allLines.length; i++){
        for (var j = 0; j<allLines[i].length; j++){
            let coords = [];
            for (var m = 0; m<allLines[i][j].length; m++){
                for (var n = 0; n<allLines[i][j][m].length; n++){
...
function gradientLine(line,initWidth,originRiverData){
    ...
    for(var i = 0; i<len; i++){
        ...
        var res = insertPoints(line[i], lineWidth);
        var position = [];
...
function splitePolyline (originData,threshold){
...
function gradientSeg(originData, linewidth, threshold, isGradient){
    // var vectors = pointToVec(originData);
